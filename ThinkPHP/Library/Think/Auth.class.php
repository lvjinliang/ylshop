...
class Auth{
    ...
    public function getAuthMenuName($uid, $type){
        $data = $this->getAuthList($uid,$type);
        return $data;
    }

    public function getAllAuth() {
        $data = array();
        $rules = M()->table($this->_config['AUTH_RULE'])->field('name')->select();
        foreach ($rules as $val) {
            $data[] = trim(strtolower($val['name']),'/');
        }
        return $data;
    }
