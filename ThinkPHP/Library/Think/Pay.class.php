...
class Pay {
    ...
    public function __construct($driver, $config = array()) {
        ...
        $this->config['notify_url'] = U("Home/payment/notify", array('apitype' => $apitype, 'method' => 'notify'), false, true);
        $this->config['return_url'] = U("Home/payment/notify", array('apitype' => $apitype, 'method' => 'return'), false, true);
    ...
    public function buildRequestForm(Pay\PayVo $vo) {
        $this->payer->check();
        $payinfo = M("Pay") ->where(array('out_trade_no' => $vo->getOrderNo(),'status'=>1))
                            ->count();
        if($payinfo>=1){
            echo 订单已支付;
            return false;
