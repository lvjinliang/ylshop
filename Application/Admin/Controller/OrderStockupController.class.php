...
class OrderStockupController extends CommonController {
    ...
    public function index() {
        ...
        $breadcrumbs[] = array('title'=>'备货订单','href'=>U('order_stockup/index'));
        ...
        $this->assign('contentUrl', U('order_stockup/ajax_get_content'));
        $this->display();
    }
    ...
    private function content () {
        ...
        $data['isShowStockupButton'] = $this->AUTH->check(trim(U('order_stockup/edit'),'/'), session('user.id'));
        ...
        return $this->fetch('Order_stockup/content');
    }
    ...
    public function edit() {
        ...
        $breadcrumbs = array();
        $breadcrumbs[] = array('title'=>'首页','href'=>U('index/index'));
        $breadcrumbs[] = array('title'=>'待确认单','href'=>U('order_stockup/index'));
        $id = I('get.id');
        ...
        $this->assign('breadcrumbs', $breadcrumbs);
        $this->display('Order_stockup/edit');
    }

    public function validate_form() {
        if (!in_array(I('post.order_status_id'),array(2,3))) {
            $this->error['order_status'] = '用户订单状态已改变,请刷新后操作';
        }

        if (I('post.order_status_id')==3) {
            $shipping_no = I('post.shipping_no');
            if (empty($shipping_no)) {
                $this->error['shipping_no'] = '请填写运单号';
            }
        }
