...
<!--Top-->
    ...
    <div id="page-content" class="register-page">
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <ul class="breadcrumb">
        ...
        <div class="row">
            <?php if(!empty($errorInfo)): ?><div id="erroInfo" class="alert  alert-danger">
                    ...
                    <strong>！</strong><?php echo ($errorInfo); ?>
                </div><?php endif; ?>
            <div class="col-sm-12">
                <div class="heading"><h2>用户登录</h2></div>
                <form name="theForm" class="form-horizontal" method="post" action="<?php echo U('Home/login/index');?>">
                    <div class="form-group <?php if(!empty($error['username'])): ?>has-error<?php endif; ?>">
                        <label for="username" class="col-sm-2 control-label">用户名：</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" placeholder="用户名" name="username" id="username" value="<?php echo ($data['username']); ?>" />
                            <?php if(!empty($error['username'])): ?><span class="help-block"><?php echo ($error['username']); ?></span><?php endif; ?>
                        </div>
                    </div>

                    <div class="form-group <?php if(!empty($error['password'])): ?>has-error<?php endif; ?>">
                        <label for="password" class="col-sm-2 control-label">密码：</label>
                        <div class="col-sm-10">
                            <input type="password" class="form-control" placeholder="密码" name="password" id="password" value="<?php echo ($data['password']); ?>" />
                            <?php if(!empty($error['password'])): ?><span class="help-block"><?php echo ($error['password']); ?></span><?php endif; ?>
                        </div>
                    </div>

                    <div class="form-group <?php if(!empty($error['verify'])): ?>has-error<?php endif; ?>">
                        <label for="verify" class="col-sm-2 control-label">验证码：</label>
                        <div class="col-sm-10">
                            <div class="row">
                            <div class="col-sm-2 col-xs-4">
                                <input type="text" class="form-control" placeholder="验证码" name="verify" id="verify" >
                            </div>
                            <div class="col-sm-10 col-xs-8">
                                <img class="verify_img" style="cursor: pointer;" src="<?php echo U('Home/public/verify');?>" data="<?php echo U('Home/public/verify');?>" onclick="changeVerify(this);">
                            </div>
                            </div>
                            <?php if(!empty($error['verify'])): ?><span class="help-block"><?php echo ($error['verify']); ?></span><?php endif; ?>
                        </div>

                    </div>
                    <input type="hidden" class="form-control" value="<?php echo ($redirect); ?>" name="redirect" id="redirect" />
                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-10">
                            <button type="submit" class="btn btn-1">登录</button>
                        </div>
                        <div class="col-sm-offset-2 col-sm-10" style="margin-top:20px;"><a href="<?php echo U('Home/register/index');?>">注册新用户</a></div>
                    </div>
                </form>
