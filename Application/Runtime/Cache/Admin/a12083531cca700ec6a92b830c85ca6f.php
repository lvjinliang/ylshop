...
<div id="main">
    ...
    <div class="main-header">
        ...
        <div class="col-sm-8">
            <div class="btn-group pull-right form-botton" role="group">
                <?php if($level <= 2): ?><a href="<?php echo U('menu/add',array('pid'=>$pid));?>" class="btn btn-primary" role="button">
                    <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>添加
                </a><?php endif; ?>
                <a href="javascript:void(0)" onclick="deleteSelect('<?php echo U('menu/delete');?>', 'id', '确认删除所选项及其子菜单')" class="btn btn-primary" role="button">
    ...
    <div class="admin_list" >
        ...
        <div class="search-bar">
            <form class="form-inline" role="form" method="get" action="<?php echo ($searchUrl); ?>">
                <div class="form-group">
                    <label for="title" class="control-label">标题</label>
                    <input type="text" class="form-control" id="title"
                           name="title" value="<?php echo ($search['title']); ?>" placeholder="标题" />
                </div>
                ...
                <div class="form-group">
                    <label for="status" class="control-label">状态</label>
                        ...
                        <select id="status" class="form-control" name="status" >
                            ...
                            <option value="1" <?php if(($search['status']) == "1"): ?>selected="selected"<?php endif; ?>>开启</option>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                ...
                <?php if(!empty($lists)): if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td>
                        <?php if($level < 2): ?><a href="<?php echo U('menu/index',array('pid'=>$list['id']));?>"><?php echo ($list['title']); ?></a>
                        <?php else: ?>
                            <?php echo ($list['title']); endif; ?>
                    </td>
                    <td><?php echo ($list['type']); ?></td>
                    <td><?php echo ($list['name']); ?></td>
                    <td><?php echo ($list['p_title']); ?></td>
                    <td><?php echo isShowToText($list['is_show']);?></td>
                    <td><?php echo statusToText($list['status']);?></td>
                    <td><?php echo ($list['sort']); ?></td>
                    <td>
                        <a href="<?php echo U('menu/update', array('id'=>$list['id']));?>" class="btn btn-primary" role="button" title="编辑">
                            <span class="glyphicon glyphicon-pencil" aria-hidden="true"> </span>
                        </a>
                        <a href="javascript:void(0);" onclick="del('<?php echo U('menu/delete');?>','<?php echo ($list['id']); ?>','id', '确认删除此项及其子菜单')"  class="btn btn-primary" role="button" title="删除">
